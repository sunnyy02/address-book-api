...
export class AddressValidationPipe implements PipeTransform<AddressDto> {
    ...
    async transform(value: AddressDto, metaData: ArgumentMetadata) {
        ...
        if(this.statePostCodeMap[value.state.toUpperCase()].substring(0,1) !== value.postCode.toString().substring(0,1)){
            throw new BadRequestException(`The postcode ${value.postCode} does not match with state: ${value.state}`);
        }
        return value;
    }
}